...
func TestCacheFromForDockerfileBuild(t *testing.T) {
	cfgWith := func(cf ...string) *config.DevContainerConfig {
		return &config.DevContainerConfig{Build: &config.BuildConfig{CacheFrom: config.StringOrStrings(cf)}}
	}
	...
	cases := []struct {
		...
		{"no build block → flag only", []string{"a"}, &config.DevContainerConfig{}, []string{"a"}},
...
func TestCacheFromForDockerfileBuildDoesNotMutateFlag(t *testing.T) {
	flag := []string{"flag1"}
	cfg := &config.DevContainerConfig{Build: &config.BuildConfig{CacheFrom: config.StringOrStrings{"cfg1"}}}
