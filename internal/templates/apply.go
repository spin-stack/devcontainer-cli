...
func FetchAndApply(params ApplyParams, selected SelectedTemplate) ([]string, error) {
	...
	if err := pfs.MkdirAll(extractDir); err != nil {
	...
	if err := pfs.WriteFile(tarPath, blobData); err != nil {
	...
	options := applyOptionDefaults(extractDir, selected.Options, logger)
	...
	err = filepath.Walk(extractDir, func(path string, info os.FileInfo, err error) error {
		...
		if err := pfs.MkdirAll(filepath.Dir(destPath)); err != nil {
			return err
		}
		if err := pfs.WriteFile(destPath, []byte(content)); err != nil {
			return err
		}
	...
	if len(selected.Features) > 0 {
		if err := mergeFeatures(params.WorkspaceFolder, selected.Features, logger); err != nil {
...
func mergeFeatures(workspaceFolder string, featureOpts []TemplateFeatureOption, logger log.Log) error {
	...
	return pfs.WriteFile(configPath, v.Pack())
}
...
func applyOptionDefaults(extractDir string, userOptions map[string]string, logger log.Log) map[string]string {
